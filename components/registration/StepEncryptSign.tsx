...
interface StepEncryptSignProps {
  ...
  onBack: () => void;
  onComplete: (txSignature: string) => void;
}
    ...
    <div>
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  state === "active" &&
                    "bg-teal/8 border border-teal animate-spin",
                  state === "pending" &&
                    "bg-border border border-border-2"
                )}
              >
      ...
      <div className="bg-card-2 border border-border-2 rounded-xl p-5 mb-6">
        ...
        {CATEGORIES.map(({ key, label, desc }) => (
          ...
          >
            <div>
              <div className="text-[13px] font-semibold text-text-secondary mb-0.5">
                {label}
              </div>
        ...
        <div className="flex items-center justify-between py-4">
          <div>
            ...
            <div className="text-[11px] text-text-muted">
              Batch notifications at 9am UTC
            </div>
          </div>
          <button
            onClick={() =>
              onPrefsChange({ ...prefs, digestMode: !prefs.digestMode })
            }
            className={cn(
              "relative w-11 h-6 rounded-full transition-colors duration-200 shrink-0 cursor-pointer",
      ...
      <div className="flex gap-3">
        <Button variant="secondary" onClick={onBack} disabled={isSigning}>
          ← Back
        </Button>
        <Button
          className="flex-1"
